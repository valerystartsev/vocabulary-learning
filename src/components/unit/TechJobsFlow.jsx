...
import {
  Cpu, UserMinus, GraduationCap, Sparkles, CheckCircle,
  ChevronRight, MessageSquare,
} from 'lucide-react';
import { useProgress } from '../../context/ProgressContext';

const ICON_MAP = { Cpu, UserMinus, GraduationCap, Sparkles };
...
// chip-style highlights. Same renderer used by ComplaintResolutionPath.
...
export default function TechJobsFlow({ data, unitId, isTeacherMode }) {
  ...
  useEffect(() => {
    if (!markedDone && seen.size >= DONE_THRESHOLD) {
      setMarkedDone(true);
      markSectionComplete?.(unitId, 'techjobsflow');
    }
          ...
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div>
              ...
              <p className="text-xs italic mt-0.5" style={{ color: 'var(--col-muted)' }}>
                Stage {steps.indexOf(active) + 1} of {steps.length} · {active.titleRu}
              </p>
            </div>
          </div>
        ...
        {markedDone && (
          ...
          >
            ...
            <p className="text-sm font-medium" style={{ color: 'var(--col-accent-text)' }}>
              You've followed technology's full ripple through the labour market.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
