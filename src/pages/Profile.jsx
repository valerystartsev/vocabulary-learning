...
// ИСПРАВЛЕНИЯ:
//
//  [FIX-1] Добавлен импорт supabase — без него handleSave не работал бы.
//
//  [FIX-2] handleSave: функция сохранения теперь реально записывает данные
//          в Supabase. Раньше там был только console.log (сохранение
//          было намеренно отключено во время разработки).
//
//  [FIX-3] Чекбокс "Financial University": удалён onClick с визуального <div>.
//          Раньше клик по квадратику срабатывал дважды и возвращал галочку
//          в исходное состояние — поэтому работало только нажатие на текст.
//          Теперь переключение идёт через label → input → onChange (один раз).
// ─────────────────────────────────────────────────────────────────────────────

// [FIX-1] Импорт supabase — нужен для handleSave
...
export default function Profile() {
  ...
  // При загрузке заполняем поля из данных пользователя
  useEffect(() => {
    if (user) {
  ...
  // Кнопка "Restart Tour" — сбрасывает флаг онбординга
  ...
  // Показываем спиннер пока данные пользователя не загружены
  if (!user) {
    return (
  ...
  // ─── [FIX-2] Функция сохранения профиля ──────────────────────────────────
  // Было: console.log — данные никуда не сохранялись.
  // Стало: upsert в таблицу profiles в Supabase.
  //
  // Важно: колонки display_name и university_tracking должны существовать
  // в таблице profiles — они добавляются через SQL-миграцию (шаг 4 гайда).
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('profiles')
        .upsert({
          id: user.id,                                        // первичный ключ
          display_name: trimmed || null,                      // имя из поля ввода
          university_tracking: Boolean(isFinancialUniversity), // состояние чекбокса
          updated_at: new Date().toISOString(),               // дата обновления
        });

      if (error) throw error;

      await refreshUser(); // обновляем данные пользователя в памяти приложения
      setSaved(true);
  ...
  return (
    <div
      className="max-w-xl mx-auto px-4 md:px-8 py-8"
    ...
    >
      ...
      <p className="text-sm mb-7" style={{ color: 'var(--col-muted)' }}>
        Профиль и настройки
      </p>
      ...
      <div className="space-y-5">
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-xs mt-1.5 italic" style={{ color: 'var(--col-muted)' }}>
                This name is visible to your teacher and shown in the app. / Это имя видит преподаватель.
              </p>
              {user.full_name && !user.displayName && (
                ...
                >
                  Currently using your registration name: <strong>{user.full_name}</strong>.
                  Edit above to set a custom display name.
                </p>
              )}
            </div>
        ...
        >
          ...
          {/*
            ...
            <label> оборачивает весь блок. Клик в любом месте — по квадратику
            или по тексту — идёт через label → активирует скрытый <input> →
            onChange меняет состояние ровно один раз.

            [FIX-3] Визуальный <div> больше НЕ имеет onClick.
            Раньше он тоже реагировал на клик — получалось два срабатывания
            подряд, галочка возвращалась в исходное состояние, и казалось
            что квадратик "не работает". Теперь onClick убран.
          */}
          <label className="flex items-start gap-3 cursor-pointer">
            <div className="relative mt-0.5">

              {/* Настоящий чекбокс — скрыт, но активируется через <label> */}
              <input
                type="checkbox"
                checked={isFinancialUniversity}
              ...
              <div
                ...
                style={{
                  backgroundColor: isFinancialUniversity
                    ? 'var(--col-accent)'
                    : 'var(--col-surface-secondary)',
                  border: `2px solid ${
                    isFinancialUniversity ? 'var(--col-accent)' : 'var(--col-border)'
                  }`,
                }}
              >
          ...
          >
            <p className="text-xs" style={{ color: 'var(--col-accent-text)' }}>
              If you check this box, your teacher can see your progress and exercise
              results in the teacher dashboard.
            </p>
            <p
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button
                ...
                style={{
                  backgroundColor:
                    theme === t.id ? 'var(--col-accent-light)' : 'var(--col-surface-secondary)',
              ...
              >
                ...
                <span
                  ...
                  style={{
                    color: theme === t.id ? 'var(--col-accent-text)' : 'var(--col-secondary)',
                  }}
                >
                  {t.label}
                </span>
        ...
        <button
          ...
          style={{
            backgroundColor: saved ? 'var(--col-correct)' : 'var(--col-accent)',
            minHeight: 52,
          }}
        >
          {saving ? (
