...
export default function Profile() {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 md:px-8 py-8" style={{ backgroundColor: 'var(--col-page-bg)' }}>
      <button
        onClick={() => navigate(-1)}
      ...
      <h1 className="font-bold text-2xl mb-1" style={{ color: 'var(--col-heading)', letterSpacing: '-0.4px' }}>
        Profile & Settings
      </h1>
      ...
      <div className="space-y-5">
        {/* Identity */}
        <div className="rounded-2xl p-5" style={{ backgroundColor: 'var(--col-surface)', border: '1px solid var(--col-border)' }}>
          <h2 className="font-semibold text-sm mb-4 flex items-center gap-2" style={{ color: 'var(--col-heading)' }}>
            <User className="h-4 w-4" style={{ color: 'var(--col-accent)' }} />
            Identity
          </h2>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--col-muted)' }}>
              ...
              {user.full_name && !user.displayName && (
                <p className="text-xs mt-1" style={{ color: 'var(--col-accent-text)', backgroundColor: 'var(--col-accent-light)', padding: '4px 8px', borderRadius: 6 }}>
                  Currently using your registration name: <strong>{user.full_name}</strong>. Edit above to set a custom display name.
                </p>
              )}
            </div>
            <div>
              ...
              <div className="px-4 py-3 rounded-xl text-sm" style={{ backgroundColor: 'var(--col-surface-secondary)', border: '1px solid var(--col-border)', color: 'var(--col-secondary)' }}>
                {user.email}
              </div>
            </div>
          </div>
        ...
        <div className="rounded-2xl p-5" style={{ backgroundColor: 'var(--col-surface)', border: '1px solid var(--col-border)' }}>
          <h2 className="font-semibold text-sm mb-3 flex items-center gap-2" style={{ color: 'var(--col-heading)' }}>
            <GraduationCap className="h-4 w-4" style={{ color: 'var(--col-accent)' }} />
            University Tracking
          </h2>
          <label className="flex items-start gap-3 cursor-pointer">
            <div className="relative mt-0.5">
              <input type="checkbox" checked={isFinancialUniversity} onChange={e => setIsFinancialUniversity(e.target.checked)} className="sr-only" />
              <div
                className="w-5 h-5 rounded flex items-center justify-center transition-all"
                style={{ backgroundColor: isFinancialUniversity ? 'var(--col-accent)' : 'var(--col-surface-secondary)', border: `2px solid ${isFinancialUniversity ? 'var(--col-accent)' : 'var(--col-border)'}` }}
                onClick={() => setIsFinancialUniversity(v => !v)}
              >
                {isFinancialUniversity && <CheckCircle className="h-3.5 w-3.5 text-white" />}
              </div>
            </div>
            <div>
              <p className="text-sm font-medium" style={{ color: 'var(--col-body)' }}>I am studying at the Financial University</p>
              <p className="text-xs mt-0.5 italic" style={{ color: 'var(--col-secondary)' }}>Я обучаюсь в Финансовом университете</p>
            </div>
          </label>
          <div className="mt-3 px-4 py-3 rounded-xl" style={{ backgroundColor: 'var(--col-accent-light)', border: '1px solid var(--col-divider)' }}>
            <p className="text-xs" style={{ color: 'var(--col-accent-text)' }}>
              If you check this box, your teacher can see your progress and exercise results in the teacher dashboard.
            </p>
            <p className="text-xs mt-1 italic" style={{ color: 'var(--col-accent-text)', opacity: 0.75 }}>
              Если вы отметите этот пункт, преподаватель будет видеть ваш прогресс и результаты заданий.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button key={t.id} onClick={() => setTheme(t.id)}
                ...
                <span className="text-xs font-semibold" style={{ color: theme === t.id ? 'var(--col-accent-text)' : 'var(--col-secondary)' }}>{t.label}</span>
        ...
        {/* Tour */}
        <div className="rounded-2xl p-5" style={{ backgroundColor: 'var(--col-surface)', border: '1px solid var(--col-border)' }}>
          <h2 className="font-semibold text-sm mb-3" style={{ color: 'var(--col-heading)' }}>App Tour / Тур по приложению</h2>
          <button
